...
async def proxy_google_login(req: GoogleLoginRequest):
    ...
    # Step 3: C# googleLogin failed — fallback to classic login
    try:
        async with httpx.AsyncClient(timeout=TIMEOUT) as client:
            # Generate a strong random password for new accounts
            ...
            login_resp = await client.post(
                ...
                json={"username": email, "password": "__google_oauth__"},
                headers={"Content-Type": "application/json", "Origin": "https://zont.cab", "Referer": "https://zont.cab/"},
            )

            if login_resp.status_code != 200:
                # User doesn't exist or wrong password — register new account
                ...
                if reg_resp.status_code != 200:
                    # Registration failed — maybe email already exists with a different password
                    ...
                    if isinstance(err, dict):
                        for v in err.values():
                            error_msg = v[0] if isinstance(v, list) else str(v)
                            break
                    raise HTTPException(status_code=400, detail=error_msg or "Account already exists with this email. Please sign in with your password.")
            ...
            raise HTTPException(status_code=401, detail="Google login failed. Please try signing in with email and password.")
