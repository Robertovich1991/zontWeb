...
import { Car, Loader2, Clock, CheckCircle, XCircle, AlertCircle, ChevronDown, ChevronUp, Navigation } from 'lucide-react';
    ...
    <div data-testid="rides-manager">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-gray-900">Courses Partenaires</h1>
          <span className="bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded-full">{rides.length}</span>
        </div>
        <div className="flex gap-2 flex-wrap">
          {['all', 'pending', 'submitted_csharp', 'accepted', 'rejected', 'completed'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${filter === f ? 'bg-emerald-500 text-white' : 'bg-gray-100 text-gray-500 hover:text-gray-900'}`}>
              {f === 'all' ? 'Toutes' : statusConfig[f]?.label || f}
              {f === 'pending' && rides.filter(r => r.status === 'pending').length > 0 && (
                <span className="ml-1 bg-yellow-500 text-white rounded-full px-1.5 text-xs">{rides.filter(r => r.status === 'pending').length}</span>
              )}
            </button>
          ))}
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-400">Aucune course</div>
      ) : (
        <div className="space-y-3">
          {filtered.map(ride => {
            ...
            return (
              <div key={ride.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden" data-testid={`ride-${ride.id}`}>
                <div className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-100/50 transition" onClick={() => setExpandedId(expanded ? null : ride.id)}>
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-emerald-600 font-bold text-sm">{ride.proposed_price} {ride.currency}</span>
                    <span className="bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded">{ride.vehicle_category_name}</span>
                ...
                {expanded && (
                  <div className="border-t border-gray-200 p-4 bg-white/50">
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      {['pending', 'accepted', 'rejected', 'completed', 'cancelled'].map(s => (
                        <button key={s} onClick={() => updateStatus(ride.id, s)} disabled={ride.status === s}
                          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${ride.status === s ? 'bg-emerald-500 text-white' : 'bg-gray-100 text-gray-500 hover:text-gray-900'}`}>
                    ...
                    <textarea placeholder="Notes admin..." defaultValue={ride.admin_notes || ''}
                      ...
                      className="w-full bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm resize-none" rows={2} />
                  </div>
                )}
              </div>
            );
